...
public class MySecureDataConteinerTM<E> implements  SecureDataContainer<E>{
	//FA:  Id --> contenitore
	// dove  n = listaUtenti.size() e 0 <= a < n
	//IR: listaUtenti != null 
	//	  for all i. 0 <= i < listaUtenti.size() => Id_i != null 
	//    for all j,i.0 <= i < j < listaUtenti.size() => Id_i != Id_j	
	private TreeMap<String,contenitore<E>> listaUtenti;
	
	public MySecureDataConteinerTM() throws IllegalArgumentException{
		listaUtenti = new TreeMap<String,contenitore<E>>();
	}
